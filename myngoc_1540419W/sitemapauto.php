<?php 
   ...
   function url($url){
		...
		$link = array("dichvu"=>"dich-vu","news"=>"tin-tuc","tailieu"=>"dao-tao","chinhsach"=>"chinh-sach");
		...
		$sitemap->addUrl(url("thuc-don"),date('c'),'daily','1');



		$sitemap->addUrl(url("tin-tuc"),date('c'),'daily','1');

		$sitemap->addUrl(url("hinh-anh"),date('c'),'daily','1');
	...
	/**
		$sql = "select * from #_product_cat where hienthi=1 and (com='product' or com='in-an-product') order by stt,id desc";		
		$d->query($sql);
	...
	**/
		...
		for($i=2;$i<=ceil(count($product)/$c);$i++){
			$sitemap->addUrl(url("thuc-don/page=".$i),date('c'),'daily','1');
		}
