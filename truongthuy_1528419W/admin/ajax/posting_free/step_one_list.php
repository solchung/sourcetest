<?php 
	...
	$d->reset();
	$sql = "select ten_vi,tenkhongdau,id,id_hangxe,id_tenxe,id_mauxe,id_loaibds,dientich,mucdich from #_product where hienthi=1 and id_list='".$id_thuoctinh."' and id='".$id_raovat."'  order by id desc";
	$d->query($sql);
	$item = $d->fetch_array();
	}
	...
	
	{
		...
		$d->reset();
	$sql = "select ten_vi,tenkhongdau,id,id_hangxe,id_tenxe,id_mauxe,id_loaibds,dientich,mucdich from #_product where hienthi=1 and id_list='".$id_thuoctinh."'   order by id desc";
	$d->query($sql);
	$item = $d->fetch_array();	
...
<div class="formRow check_menu_adv">
  	...
  	<label>Diện tích </label>          
       <div class="formRight">
			...
			<div class="selector">
				<input type="text" value="<?=@$item['dientich']?>" name="dientich" title="Nhập diện tích" class="tipS" />
			</div><!--end selector-->
