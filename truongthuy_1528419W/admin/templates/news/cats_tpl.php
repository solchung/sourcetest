...
<td colspan="10"><div class="pagination">  <?=pagesListLimitadmin($url_link , $totalRows , $pageSize, $offset)?>     </div></td>
    ...
    <tbody>
       ...
       <input type="checkbox" name="chon[]" id="chon" value="<?=$items[$i]['id']?>" class="chon" />
        ...
        <td align="center">
            <input type="text" value="<?=$items[$i]['stt']?>" name="ordering[]" onkeypress="return OnlyNumber(event)" class="tipS smallText" original-title="Nhập số thứ tự" id="number<?=$items[$i]['id']?>" onchange="return updateNumber('news_cat', '<?=$items[$i]['id']?>')" />
		...
		?>
            ...
            <?php } else { ?>
         <a href="index.php?com=news&act=man_cat&typeparent=<?=$_GET['typeparent']?>&hienthi=<?=$items[$i]['id']?>" title="" class="smallButton tipS" original-title="Click để hiện"><img src="./images/icons/color/hide.png" alt=""></a>
         <?php } ?>
        </td>
        
        <td class="actBtns">
            <a href="index.php?com=news&act=edit_cat&typeparent=<?=$_GET['typeparent']?>&id_list=<?=$items[$i]['id_list']?>&id=<?=$items[$i]['id']?>&id_thuoctinh=<?=$items[$i]['id_thuoctinh']?>" title="" class="smallButton tipS" original-title="Sửa"><img src="./images/icons/dark/pencil.png" alt=""></a>
            <a href="index.php?com=news&act=delete_cat&typeparent=<?=$_GET['typeparent']?>&id=<?=$items[$i]['id']?>&id_thuoctinh=<?=$items[$i]['id_thuoctinh']?>" onclick="CheckDelete('index.php?com=news&act=delete_cat&typeparent=<?=$_GET['typeparent']?>&id=<?=$items[$i]['id']?>'); return false;" title="" class="smallButton tipS" original-title="Xóa"><img src="./images/icons/dark/close.png" alt=""></a>        </td>
      </tr>
         <?php } ?>
                </tbody>
